...
module.exports.run = async function({ api, event, args, __GLOBAL, client }) {
	...
	var body, songInfo, timePlay;
	
	if (urlValid) {
		try {
			songInfo = await ytdl.getInfo(args[0]);
			body = `Tiêu .đề: ${songInfo.videoDetails.title} | [${(songInfo.videoDetails.lengthSeconds - (songInfo.videoDetails.lengthSeconds %= 60)) / 60 + (9 < songInfo.videoDetails.lengthSeconds ? ':' : ':0') + songInfo.videoDetails.lengthSeconds}]`;
		}
		catch (error) {
			api.sendMessage("Thông tin của YouTube đã xảy ra sự cố, lỗi: " + error.message, event.threadID, event.messageID);
		}
		return ffmpeg().input(await ytdl(args[0])).toFormat("mp3").pipe(createWriteStream(__dirname + "/cache/music.mp3")).on("close", () => api.sendMessage({attachment: createReadStream(__dirname + "/cache/music.mp3")}, event.threadID, () => unlinkSync(__dirname + "/cache/music.mp3"), event.messageID));

	}
	if (scRegex.test(args[0]) || mobileScRegex.test(args[0])) {
		try {
			songInfo = await scdl.getInfo(args[0], __GLOBAL.settings.SOUNDCLOUD_API);
			let timePlay = Math.ceil(songInfo.duration / 1000);
	...
	try {
		...
		for (let value of results) {
			...
			link.push(value.id);
			songInfo = await ytdl.getInfo(value.id);
